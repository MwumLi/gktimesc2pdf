...
(async () => {
  ...
  if (config.merge) {
    console.log('开始合并')
    mergePdf(articles, config.mergeFilename);
    console.log('合并结束')
  }
  await browser.close();
  console.log("finish");
...
function getBookMenuIds() {
  ...
  const postData =
    `{"cid": ${config.cid},"size":300,"prev":0,"order":"earliest","sample":false}`;
  const options = {
...
async function exportArticle(page, articles) {
  for (let item of articles) {
    ...
    item.path = path.join(config.dir, `${item.name}.pdf`);
    await page.pdf({
      path: item.path
    });
...
function loadConfig(configFile) {
  ...
  config.mergeFilename = path.join(config.dir, config.mergeFilename)
  return config
}
